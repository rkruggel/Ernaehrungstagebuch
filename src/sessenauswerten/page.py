...
FetchEssenEntries = Callable[[str, str], list[dict[str, str]]]
...
def register_essen_analysis_pages(
    build_shell: Callable[[str], None],
    fetch_essen_entries: FetchEssenEntries,
    update_essen_entry: UpdateEssenEntry,
    delete_essen_entry: DeleteEssenEntry,
) -> None:
    @ui.page('/essen-auswerten')
    def essen_analysis_page() -> None:
        build_shell('Essen Auswerten')

        today = date.today().isoformat()

        with ui.column().classes('min-h-screen w-full items-center gap-5 px-6 py-10'):
            ui.label('Essen Auswerten').classes('text-3xl font-bold text-slate-800 text-center')

            with ui.row().classes('w-full max-w-4xl flex-wrap items-end justify-center gap-4'):
            ...
            def load_entries() -> None:
                ...
                try:
                    rows = fetch_essen_entries(date_from, date_to)
            ...
            with ui.dialog() as edit_dialog, ui.card().classes('w-[420px] max-w-full gap-3'):
                ...
                edit_time_input = ui.input('Zeit').props('type=time step=900 dense') \
                    .classes('w-full')
                edit_meal_select = ui.select(
                    MEAL_OPTIONS,
                    label='Mahlzeit',
                ).props('dense options-dense').classes('w-full')
                edit_food_input = ui.input('Was gegessen').props('dense').classes('w-full')
                edit_place_input = ui.input('Wo gegessen').props('dense').classes('w-full')
            ...
            def open_edit(row: dict[str, str]) -> None:
                ...
                edit_meal_select.value = row.get('mahlzeit', '')
                edit_food_input.value = row.get('was_gegessen', '')
                edit_place_input.value = row.get('wo_gegessen', '')
                edit_dialog.open()
            ...
            def save_edit() -> None:
                ...
                document = {
                    ...
                    'mahlzeit': str(edit_meal_select.value or ''),
                    'was_gegessen': str(edit_food_input.value or '').strip(),
                    'wo_gegessen': str(edit_place_input.value or '').strip(),
                }
                if not all(document.values()):
                    ui.notify('Bitte alle Felder ausfuellen.', color='warning')
                    return
                try:
                    update_essen_entry(row_id, document)
                except Exception as exc:
            ...
            def open_delete(row: dict[str, str]) -> None:
                ...
                delete_label.set_text(
                    f"{row.get('datum', '')} {row.get('zeit', '')} {row.get('was_gegessen', '')}"
                )
                delete_dialog.open()

            def confirm_delete() -> None:
                try:
                    delete_essen_entry(delete_id['value'])
                except Exception as exc:
