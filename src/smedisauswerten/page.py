...
FetchMedisEntries = Callable[[str, str], list[dict[str, str]]]
UpdateMedisEntry = Callable[[str, dict[str, object]], None]
DeleteMedisEntry = Callable[[str], None]
FetchOptions = Callable[[], list[str]]
SaveOption = Callable[[str], list[str]]

MEDIS_ANALYSIS_COLOR = '#34497F'
...
def register_medis_analysis_pages(
    build_shell: Callable[[str], None],
    fetch_medis_entries: FetchMedisEntries,
    update_medis_entry: UpdateMedisEntry,
    delete_medis_entry: DeleteMedisEntry,
    fetch_medications: FetchOptions,
    add_medication: SaveOption,
) -> None:
    @ui.page('/medis-auswerten')
    def medis_analysis_page() -> None:
        build_shell('Medis Auswerten')

        today = date.today().isoformat()

        with ui.column().classes('min-h-screen w-full items-center gap-5 px-6 py-10'):
            ui.label('Medis Auswerten').classes('text-3xl font-bold text-slate-800 text-center')
            ...
            with ui.row().classes('w-full max-w-2xl flex-wrap items-end justify-center gap-4'):
                ...
                with ui.button_group().props('unelevated'):
                    ui.button(icon='chevron_left', on_click=lambda: shift_date_range(-1)) \
                        ...
                        .style(f'background: {MEDIS_ANALYSIS_COLOR} !important; color: white;')
            ...
            def visible_columns() -> list[dict[str, str]]:
                ...
                return [*columns, medication_column, action_column]
            ...
            def load_entries() -> None:
                ...
                try:
                    rows = fetch_medis_entries(date_from, date_to)
            ...
            with ui.dialog() as edit_dialog, ui.card().classes('w-[360px] max-w-full gap-3'):
                ui.label('Medis ändern').classes('text-lg font-semibold text-slate-900')
                edit_id = {'value': ''}
                ...
                edit_time_input = ui.input('Zeit').props('type=time step=900 dense') \
                    .classes('w-full')
                edit_medication_select = ui.select(
                    [],
                    label='Medikament',
            ...
            def medication_options_with_current(current_value: str) -> list[str]:
                try:
                    options = fetch_medications()
                except Exception as exc:
                    ui.notify(f'Medikamente konnten nicht geladen werden: {exc}', color='warning')
                    options = []
                return prioritize_option(clean_options([*options, current_value]), current_value)

            def open_edit(row: dict[str, str]) -> None:
                medication = str(row.get('medikament') or '').strip()
                edit_id['value'] = str(row.get('row_key', ''))
                edit_date_input.value = row.get('datum', '')
                edit_time_input.value = quarter_hour_time(row.get('zeit', ''))
                edit_medication_select.set_options(
                    medication_options_with_current(medication),
                    value=medication or None,
                )
                edit_dialog.open()

            def save_edit() -> None:
                row_id = edit_id['value']
                medication = str(edit_medication_select.value or '').strip()
                document = {
                    'datum': str(edit_date_input.value or ''),
                    'zeit': quarter_hour_time(edit_time_input.value),
                    'medikament': medication,
                }
                if not document['datum'] or not document['zeit'] or not document['medikament']:
                    ui.notify('Bitte alle Felder ausfuellen.', color='warning')
                    return
                try:
                    add_medication(medication)
                    update_medis_entry(row_id, document)
                except Exception as exc:
            ...
            def open_delete(row: dict[str, str]) -> None:
                ...
                delete_label.set_text(
                    f"{row.get('datum', '')} {row.get('zeit', '')} {row.get('medikament', '')}"
                )
                delete_dialog.open()

            def confirm_delete() -> None:
                try:
                    delete_medis_entry(delete_id['value'])
                except Exception as exc:
